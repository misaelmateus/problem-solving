...
bool cw(pt a, pt b, pt c){ return cmp(signed_area_plus_2(a, b, c)) > 0; }
bool ccw(pt a, pt b, pt c){ return cmp(signed_area_plus_2(a, b, c)) < 0; }
bool colinear(pt a, pt b, pt c){ return !cmp(signed_area_plus_2(a, b, c)); }
...
int main(){
	...
	while((n = readInt()) && n+1){
        ...
        for(int i = 0; i < hull.size()-2; i++){
            int l = i+1;
            for(int j = i+2; j < hull.size()-1; j++){
                area = abs(signed_area_plus_2(hull[i], hull[j], hull[l]));
                while(area1 = abs(signed_area_plus_2(hull[i], hull[j], hull[++l])) > area){
                    area = area1;
                }
                l--;
                ans = max(ans, area);
